$(function(){
	...
	$(".saveBtnAnnouncement").click(function(e){
		...
		var subj = $("input:text[name='info_subject']").val();
		var date = $(".info_display_date").val();
		var content = $(".info_content").val();

		if(subj != "" || date != "" || content != "" ){
	  		...
	  		success:function(data){
	  			...
	  			if (data.success == 1) {
	  				...
	  				if(res[0].position_id == 1) { //Admin
						...
						$(".a_eposted_on").val(moment(res[0].posted_on).format("MM/DD/YYYY"));
						$(".a_eposted_until").val(moment(res[0].display_until).format("MM/DD/YYYY"));
