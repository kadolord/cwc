$(function(){
	...
	$('#application_category-form').on('keyup keypress', function(e) {
	...
	$(".saveBtnAccounts").click(function(e){
		...
		var thiss = $("#application_category-form");
	  		...
	  		success:function(data){
	  			...
	  			if (data.success == 1) {
	  				...
	  				$(".application_id").val(res[0].application_id); //user_id pk
	  					...
	  					var a = accounting.formatMoney(res[0].convenience_fee,"₱",2);
	  					var b = accounting.formatMoney(res[0].application_fee,"₱",2);

	  					$(".info_aname").val(res[0].description);
						$(".info_pfee").val(res[0].convenience_fee);
		  				$(".info_bfee").val(res[0].application_fee);
	...
	$('.btnClickAddAccount').click(function(){
		$('.info_aname').prop('autofocus', true);
		$('.info_aname').val('');
